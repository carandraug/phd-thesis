...
acquisition and analysis as an integral part of a researchers work.
...
there is limited hope for other scientists to recreate the implementation.  This
makes access to the original software used in research a prerequesite for
reproducibility.

Sharing the code is not enough.  Programs that undertake the analaysis
...
Following on free software came the ideals of open and collaborative
...
methods of free software development by an exclusive group, in the
``Cathedral'',  where the source code is only made public for releases,
with a method where development
happens in the public domain, the ``Bazaar'', where anyone can see
...
through discussion about development in public forums, inviting
...
have a personal interest on enabling the
...
than writing their own project just to have the feature
they lack.

Free software projects such as academic free software
projects \citep{schindelin2012fiji, bioperl} are now
now almost all developed in this way.
...
Reproducible research has two principal motivations.  One is to allow other
...
methodology.  This is the essential replicability requirement of science.
The same analysis can also be performed on other
datasets.  When associated with the freedom to inspect the
source code, reproducible research also enables anyone
to check for errors directly in the
code.  All software used in this thesis is free and open source.

The second reason, and the one of most important concern in this chapter, is to
enable others to use the tools and insights
developed for new research directions.  This
necessitates a different approach to the development of scientific
software requiring more insight in the software design and engineering.
...
releases was implemented with extra capabilities that are not yet needed.
They could have also even been implemented
directly in the BioPerl plugin as configurable options.  However, having
them as separate plugin components enables other software projects to use
them.

As free software, there is no sales data and
we do not know exactly how many people use
our work since we only hear when parts don't work.  We do
...
when users with newer versions of BioPerl
...
become entangled with that of other contributors
...
but the availability of the same functionality as a library allows
reuse by other developers.
This can be addressed by writing code with potential to be useful for
...
a series of a functions from the FRAP package that we developed separately.
This allows other
researchers to reuse the parts they need.  For example, microscopists
can utilise
...
machine or update.  Another alternative would be to include the
dependencies in our software.  However, this has its own risks by
making the dependencies opaque.  For example, halfway during our project
NCBI moved their E-Utilities services to a encrypted protocol which was
fixed in Bio-Eutilities but would have necessitated updating our
entire \command{bp\_genbank\_ref\_extractor} component if we had
included Bio-EUtilities in our release.
