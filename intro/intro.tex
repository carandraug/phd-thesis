\section{Chromatin}
  ...
  \subsection{Histones}
    ...
    \subsubsection{Histone and nucleosome structure}
      ...
      of histone fold dimer assembly where by
    ...
    \subsubsection{Historical perspective on chromatin structure}
      ...
      \afterpage{%
        \captionofIntro{table}{Historical perspective of chromatin
          structure}{}%
  ...
  \subsection{Chromatin Dynamics}
    Although there is no internal physical compartmentalisation created by
    ...
    Brownian motion and is restricted to a small nuclear volume \addref{}.
    ...
    is dependent on the cell cycle \addref{}. Noncentromeric chromatin is highly
...
\section{Fluorescence microscopy}
  ...
  In a typical widefield microscope configured for such experiment,
  ...
  of them to be excited and emit light at an higher wavelength.  Some
  ...
  image.  The dichroic mirror is the central piece in such
  configuration since it both reflects the excitation light but passes
  ...
  \subsection{Fluorescent proteins}
    ...
    properties suited to specific microscopy techniques \addref{}.
    These are often split into two major classes:
    Synthetic fluorescent
    molecules such as DAPI or fluorescein, either bind to the molecule of
    ...
    derivatives with different colours \addref{}.
    ...
    Reversible photo-switchable FPs undergo
        interchangeable conversion between dark and bright states\addref{}.
        ...
        \todo{this is not what makes them usable for localisation
          microscopy, this is what makes them better than other
          switching options, it is the switching of subpopulations
          that is important, you can use PAGFP or chemical switchers
          for LM}
  ...
  \subsection{FRAP}
    ...
    became extensively used to obtain qualitative and quantitative
    insight into the kinetic properties of proteins after the
    ...
    insight on the dynamics of the molecule.  The recovery is a
    ...
    compartment can be bleached and recovery happens thought transport
    ...
    and binding to the complexes there.  This provides estimates for
    ...
    Finally, if binding to a complex is consider, it also has a
    plethora of potential parameters.  Multiple
...
\section{Bioinformatics}
  ...
  easily access such amounts of data in public data repositories.
  ...
  tools to handle large datasets is a race against the technology that is
  ...
  been lagging behind since 1990s so has the training of biologists
  ...
  scientists is also a tool and should also allow such a
  ...
  software as software providing the four basic freedoms: freedom to run a
  ...
  In fact,
  the free software movement began in academia itself, founded by
  Richard Stallman at the MIT Artifical Intelligence lab.  This was
  triggered by a series of events caused by non-free software, namely
  an AI lab spin-off that made its software non-free, and the lab mainframe
  being replaced with another that ran on non-free software
  ...
  The lack of access to the code in publications not only delays research by
  obliging investigators to redo the work of others but also by
  increasing the difficulty for other researchers to
  validate previous work by leaving them unable to
  reproduce the published experiments.
  For example, for a survey of 18 published microarray gene-expression
  studies, it took a team of data analysts an average of half a week to
  attempt to replicate a single figure or table, with some figures taking more
  than one week.  Only two of the 18 figures were ``reproducible in
  principle'', that is, with differences that were felt as minor
  \citep{ioannidis2009repeatability}.
  Difficulty with reproducibility
  does not imply that results have been fabricated.
  While the main problem identified was the lack of availability
  of data, the second was the lack of detail on how to perform the
  analysis.

  This reproducibility has become a growing issue in recent years and led to
  ...
  The newness of the field, explains the lack of detailed data on the
  extent of reproducibility, or lack of it thereof.

  With computers being central in the step of analysis, one would
  expect that this step could be shared in the form of source code.
  However, that is not the case.  Publications favour a natural
  language description of the analysis which lacks the level of
  detail that would allow their colleagues to repeat or improve on
  their work \citep{ince2012case}.
...
\section{Aims and Objectives}
  ...
  which are largely euchromatin.  We wished to apply FRAP to test
