\section{Chromatin}
  ...
  \subsection{Histones}
    ...
    \subsubsection{Histone and nucleosome structure}
      ...
      The linear organisation of each core histone protein is divided
  ...
  \subsection{Chromatin Dynamics}
    ...
    membranes like in cytoplasmic organelles, the chromatin within the
    ...
    using FRAP, indicated that bulk chromatin in interphase was
    ...
    major mobility on the scale of individual chromosomes, only small
    Brownian diffusion \citep{edelmann2001morphology}.  Some authors
    described curvilinear movements of chromatin but they were
    attributed to the nuclear rotation.  This apparent immobility was
    attributed to the chromatin attachment to subnuclear structures
    ...
    operating at different spacial scales and time
    \cite{hubner2010chromatin}.

    In yeast, centromeric chromatin movement is mainly caused by
    ...
    periphery relocated to the interior of the nucleus upon
    activation.  In a study, a locus that was transcriptionally active
    moved towards the interior of the nucleus at
    \SIrange{0.1}{0.9}{\micro\meter\per\min} over
    \SIrange{1}{5}{\micro\meter} \citep{chuang2006long}.  Also, inter
    ...
    Another example is the contraction and looping of the
    ...
    Correlation Spectroscopy (DCS) for bulk chromatin dynamics which
    shown that chromatin movement is organised in large regions in the
    ...
    interphase chromatin than it was appreciated at the start of the
    2000s.
...
\section{Fluorescence microscopy}
  ...
  \subsection{FRAP}
    ...
    happen.  Likewise, the bleaching event on itself is not
    instantaneous and will occur while molecules are exchanging.
    The shape of the bleached area is a parameter on its own, but the
    profile of the laser beam will also have an effect being modelled
    either as gaussian or flat-top.

    Diffusion rates may be fixed, fitted, or assumed to instantaneous,
    and a single diffusion rate may be assumed for the whole
    compartment or that non-homogeneous regions in the compartment
    cause different diffusion rates.

    Finally, the binding also has plenty of parameters.  Multiple
    ...
    the binding places may be not equal throughout the compartment.

    While these may appear parameters on the mathematical model for
    the fitting, they also affect how the experiment is performed.
    ...
    \todo[inline]{I'm not sure if you planned to put something about
      software here?}

\section{Aims and Objectives}

  Histones organise the fundamental level of chromatin structure and
  ...
  experiments and yeast.  We investigated the use of FRAP for testing
  the models of nucleosome dynamics in human cells.

  While histones are usually referred to by their type only, H2A, H2B,
  H3, or H4, they are actually part of a large family of genes whose
  sequence differences are rarely recognised.  At the very start of
  our project, we required a catalogue of the different histones in
  human but the last version of such inventory had been performed in
  2002, before the finalisation of the human genome, and was no longer
  up to date.  In \Cref{ch:histone-catalogue} we performed an up to
  date catalogue of all human core histones.  Recognising the temporal
  value of such a catalogue, we automated its creation so that a
  constantly up to date version could be performed with little effort.

  In an attempt to test FRAP as a tool for measuring the effect of point
  changes in histone sequences, we were faced with the complications
  of long time FRAP experiments.  In \Cref{ch:kill-frap} we detail our
  step-wise approach at solving these issues.

  Throughout this studies, we developed many pieces of software and
  strived to improve existing tools that would be available to any
  researcher.  In \Cref{ch:software} we describe the multiple tools we
  ...
  construction from the raw data.  This thesis thus became an exercise
  on the use of software development tools for the maintenance of a
  reproducible research project, much in the same manner one would
  maintain public software project.
