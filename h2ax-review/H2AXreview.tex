...
heterogeneity within each isoform \citep{DBMC+06,WFM+02}. Four
...
H2A.F/Z and H2ABbd \citep{WFM+02}.
...
(\fref{fig:h2ax-review:celegans}). Although these do not strictly
...
(\tref{tab:h2ax-review:H2A-localisation}).
...
in H2AFX (\fref{fig:h2ax-review:H2AFX}) compared to a single CCAAT
...
(\fref{fig:h2ax-review:H2AFX}). The combined synthesis of H2AX
...
(\fref{fig:h2ax-review:H2AX-logo}). As mentioned in
...
\setsidecappos{t}
\begin{sidecaption}[Sequence logo of human canonical H2A isoforms and H2AX]%
...
\centering
\includegraphics[height=\textheight]{h2ax-review/figs/Fig3}
\end{sidecaption}
\end{figure}
...
\fref{fig:h2ax-review:H2AInNucleosome}). Inspection of the human and
...
\fref{fig:h2ax-review:framed:b}) which tracks the minor groove at
superhelical location~4.5 (SHL4.5)
(\fref{fig:h2ax-review:framed:a}). The substitution of glutamine with
...
(\fref{fig:h2ax-review:H2AInNucleosome} and
\fref{fig:h2ax-review:framed:c}). Importantly, this residue makes
...
(\fref{fig:h2ax-review:H2AInNucleosome} and
\fref{fig:h2ax-review:framed:d}). Nevertheless, the exchange of the
...
prior acetylation at Lys\,5 (\tref{tab:h2ax-review:H2AX-PTM})
...
human H2A sequences (\fref{fig:h2ax-review:H2AX-logo}).
...
point on the nucleosome (\fref{fig:h2ax-review:H2AInNucleosome}) so
